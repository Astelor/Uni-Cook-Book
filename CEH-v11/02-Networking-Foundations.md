...
- [Cloud Computing](#cloud-computing)
  - [Advantages for cloud computing](#advantages-for-cloud-computing)
...
> The things that has yet to be organized.
...
> At the data link layer (layer 2) of the OSI model. Ethernet is the protocol we'll most likely to be worrying about.

- PDU at layer 2: a frame
  - A chunk of data with physical address in it, tis a frame.
...
> A multiport bridge.
> 
> It's the thing you have under your table, that thing's a switch, ok. 

```
    To B's MAC           Sends to B's port
[A]------------>[switch]------------------->[B]
       (where's B's MAC's port at?)
```
...
```
- Use the MAC address as the index value.
- Make MAC-to-port number look-up more efficient.

---

- 🔴 (attacker):
  - (With some control on the switch), tell the switch to mirror traffic on one port to another port.
  - Fool the switch into sending traffic to you.
...
- PDU: a packet
  - a message from the IP header down is a packet.
...
> At Transport layer (layer 4), between Application and Network. Make sure information is delivered accurately and in the correct sequence.
